...
```{r}
library(leaps)
regfit.full <- regsubsets(Rating ~. , data = Credit, nvmax = 12)
summary(regfit.full)
...
What are the coefcients selected with BIC?
...
```{r}
regfit.fwd <- regsubsets(Rating ~. , data = Credit , nvmax = 12,
method = "forward")
summary(regfit.fwd)
...
```{r}
regfit.bwd <- regsubsets(Rating ~. , data = Credit , nvmax = 12,
method = "backward")
summary(regfit.bwd)
...
test <- (!train)
regfit.best <- regsubsets(Rating ~ ., data = Credit[train, ], nvmax = 12)
test.mat <- model.matrix(Rating ~ ., data = Credit[test, ])
val.errors <- numeric(12)
for(i in 1:12){
  coefi <- coef(regfit.best, id = i)
  pred <- test.mat[, names(coefi)]%*%coefi
  val.errors[i] <- mean((Credit$Rating[test] - pred)^2)
}
val.errors
...
## Creating the `regsubsets` `predict` function
...
#
for(j in 1:k){
  best.fit <- regsubsets(Rating ~ ., data = Credit[folds != j, ], nvmax = 12)
  for(i in 1:12){
    pred <- predict(best.fit, Credit[folds ==j,], id = i)
    cv.errors[j, i] <- mean((Credit$Rating[folds==j] - pred)^2)
  }
}
...
Note that the best model contains 2 variables.  We now perform best subset selection of the full data set in order to obtain the 2-variable model.

```{r}
reg.best <- regsubsets(Rating ~ ., data = Credit, nvmax = 12)
coef(reg.best, 2)
```
